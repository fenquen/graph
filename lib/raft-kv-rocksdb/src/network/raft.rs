...
impl Raft {
    ...
    pub async fn append(&self,
                        req: AppendEntriesRequest<TypeConfig>) -> Result<AppendEntriesResponse<u64>, toy_rpc::Error> {
        tracing::debug!("handle append");
        self.app.raft.append_entries(req).await.map_err(|e| toy_rpc::Error::Internal(Box::new(e)))
    }

    #[export_method]
    pub async fn snapshot(&self, req: InstallSnapshotRequest<TypeConfig>) -> Result<InstallSnapshotResponse<u64>, toy_rpc::Error> {
