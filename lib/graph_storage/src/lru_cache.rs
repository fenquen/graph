...
pub(crate) struct LruCache {
    ...
    pub(crate) pageId2Page: HashMap<PageId, Arc<RwLock<Page>>>,
...
impl LruCache {
    pub(crate) fn new(capacity: usize) -> LruCache {
        LruCache {
            capacity,
            pageId2Page: HashMap::with_capacity(capacity),
    ...
    pub(crate) fn set(&mut self, pageId: PageId, page: Arc<RwLock<Page>>) {
        if let None = self.pageId2Page.insert(pageId, page) {
            self.leastRecentPageIds.push_back(pageId);
        }

        if self.pageId2Page.len() > self.capacity {
                ...
                if let Some(pageId) = self.leastRecentPageIds.pop_front() {
                    if let Some(page) = self.pageId2Page.get(&pageId) {
                        if Arc::strong_count(page) > 1 {
                            ...
                            self.pageId2Page.remove(&pageId);
                        }
                    }
                }
            }
    ...
    #[inline]
    pub(crate) fn remove(&mut self, pageId: PageId) -> Option<Arc<RwLock<Page>>> {
        self.pageId2Page.remove(&pageId)
    }
}