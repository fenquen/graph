...
use crate::{byte_slice_to_u64, file_goto_start, suffix_plus_plus, throw, throwFormat, u64ToByteArrRef};
...
lazy_static! {
    pub static ref STORE: TrickyContainer<DB> = TrickyContainer::new();
...
pub const KEY_PREFIX_KEY_2_ORIGIN_DATA_KEY: KeyPrefix = 3;
...
/// 普通的数据(key的前缀是KEY_PREFIX_DATA)来说是 prefix 4bit + rowId 60bit
...
pub const MVCC_KEY_BYTE_LEN: usize = DATA_KEY_BYTE_LEN + KEY_TAG_BYTE_LEN + TX_ID_BYTE_LEN;
...
/// 单独用来保存txId的columnFamily
/// txId -> empty
pub const COLUMN_FAMILY_NAME_TX_ID: &str = "TX_ID";

/// 单独用来保存表之类的元数据的
/// tableId(数字) -> dbObject的json文本
pub const COLUMN_FAMILY_NAME_META: &str = "META";

pub fn isVisible(currentTxId: TxId, xmin: TxId, xmax: TxId) -> bool {
    // invisible
    if currentTxId >= xmax {
        if (xmax != TX_ID_INVALID) {
            return false;
        }
    }
...
pub fn init() -> Result<()> {
    ...
    if std::fs::exists(dbDataDir)? == false {
        log::info!("database directory {} not exist, will create", dbDataDir);

        std::fs::create_dir_all(dbDataDir)?;

        let db = DB::open(&rocksDbOpts, dbDataDir)?;

        // 创建名为"META"和"TX_ID"的columnFamily
        // 不是普通的表,不用数字id的文本为columnFamily的name的
        db.create_cf(COLUMN_FAMILY_NAME_META, &*global::DEFAULT_ROCKS_DB_OPTS)?;
        db.create_cf(COLUMN_FAMILY_NAME_TX_ID, &*global::DEFAULT_ROCKS_DB_OPTS)?;
    ...
    let cfDescs =
        existingCFNames.iter().map(
            |cfName| -> ColumnFamilyDescriptor{
                log::info!("column family: {}", cfName);
                ColumnFamilyDescriptor::new(cfName, Options::default())
            }
        ).collect::<Vec<ColumnFamilyDescriptor>>();

    let db = DB::open_cf_descriptors(&rocksDbOpts, dbDataDir, cfDescs)?;
    ...
    let dbObjectVec = {
        ...
        let columnFamilyMeta = db.cf_handle(COLUMN_FAMILY_NAME_META).unwrap();

        for iterResult in db.iterator_cf(&columnFamilyMeta, IteratorMode::Start) {
            let (key, value) = iterResult?;

            // META的key是id
            let dbObjectId = byte_slice_to_u64!(&*key);

            // META的value是DBObject的json文本
    ...
    for dbObject in dbObjectVec {
        ...
        let columnFamilyName = dbObject.getCFName();

        let columnFamily = db.cf_handle(&columnFamilyName).unwrap();
        let mut rawIterator: DBRawIterator = db.raw_iterator_cf(&columnFamily);
        ...
        match (rawIterator.key(), columnFamilyName.as_str()) {
            ...
            (None, _) => dbObject.getRowIdCounter()?.store(ROW_ID_MIN, Ordering::Release),
        }
...
mod test {
    ...
    use rocksdb::{DB, OptimisticTransactionDB, Options, TransactionDB, WriteBatchWithTransaction, ColumnFamilyDescriptor};
    ...
    pub fn testRocksDB() {
        // db数据要求存在不然报错的
        let cfNames = DB::list_cf(&Options::default(), "test").unwrap();
